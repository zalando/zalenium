...
DockerTerminate()
{
    ...
    if [ "$SEND_ANONYMOUS_USAGE_INFO" = true ]; then
        ...
        DOCKER_ENGINE_ID=$(docker info 2>&1 | grep -Po '(?<=^ID: )(\w{4}:.+)')
...
StartUp()
{
    ...
    # Random ID generated by each docker installation, not related to the user nor the machine
    DOCKER_ENGINE_ID=$(grep -Po '(?<=^ID: )(\w{4}:.+)' docker_info.txt)
    ...
    export ZALENIUM_GA_ANONYMOUS_CLIENT_ID=${DOCKER_ENGINE_ID}
