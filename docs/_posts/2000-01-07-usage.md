...
Si vous souhaitez connaitre l'état du GRID avec un niveau de détail plus important, vous pouvez utiliser le path suivant :
...
{% endhighlight %}
Grid disponible avec au moins un noeud de rattaché :
{% highlight shell %}
    curl -sSL http://localhost:4444/grid/status | jq .value | grep AVAILABLE
{% endhighlight %}
Grid disponible avec avec tous les noeuds demandés à l'init de rattachés :
{% highlight shell %}
