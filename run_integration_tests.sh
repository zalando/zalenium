...
if [ "$TRAVIS_PULL_REQUEST" = "false" ] && [ -n "${TRAVIS_TAG}" ] && [ "${TRAVIS_TAG}" != "latest" ]; then
    ...
    if [ "$INTEGRATION_TO_TEST" = browserStack ]; then
        if [ -n "${BROWSER_STACK_USER}" ]; then
            ...
            mvn clean verify -Dlogback.appender=STDOUT -Dlogback.configurationFile=logback.xml -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -Dskip.failsafe.setup=true -DintegrationToTest=${INTEGRATION_TO_TEST}
            # Check for generated videos
    ...
    if [ "$INTEGRATION_TO_TEST" = dockerCompose ]; then
        if [ -n "${SAUCE_USERNAME}" ]; then
            ...
            mvn clean verify -Dlogback.appender=STDOUT -Dlogback.configurationFile=logback.xml -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -Dskip.failsafe.setup=true -DintegrationToTest=sauceLabs
            # Check for generated videos
            ls -la ${VIDEOS_FOLDER}/saucelabs*.mp4 || (echo "No Sauce Labs videos were downloaded." && exit 2)
            ls -la ${VIDEOS_FOLDER}/zalenium*.mp4 || (echo "No Zalenium videos were generated." && exit 2)
    ...
    if [ "$INTEGRATION_TO_TEST" = minikube ]; then
        ...
        mvn clean verify -Dlogback.appender=STDOUT -Dlogback.configurationFile=logback.xml -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -Dskip.failsafe.setup=true -Dgroups=minikube
    fi
fi
