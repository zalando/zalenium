...
if [ "$TRAVIS_PULL_REQUEST" = "false" ] && [ -n "${TRAVIS_TAG}" ] && [ "${TRAVIS_TAG}" != "latest" ]; then
    ...
    if [ "${INTEGRATION_TO_TEST}" = sauceLabs ]; then
        if [ -n "${SAUCE_USERNAME}" ]; then
            ...
            mvn clean verify -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -DintegrationToTest=${INTEGRATION_TO_TEST} -Dexcludegroups=videoSplit
    ...
    if [ "${INTEGRATION_TO_TEST}" = dockerCompose ]; then
        if [ -n "${SAUCE_USERNAME}" ]; then
            ...
            mvn verify -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -Dskip.failsafe.setup=true -DintegrationToTest=sauceLabs -Dexcludegroups=videoSplit
            # Check for generated videos
            ls -la ${VIDEOS_FOLDER}/saucelabs*.mp4 || (echo "No Sauce Labs videos were downloaded." && exit 2)
            ls -la ${VIDEOS_FOLDER}/zalenium*.mp4 || (echo "No Zalenium videos were generated." && exit 2)
    ...
    if [ "${INTEGRATION_TO_TEST}" = minikube ]; then
        ...
        mvn verify -Pintegration-test -DthreadCountProperty=2 -Dskip.surefire.tests=true -Dskip.failsafe.setup=true -Dgroups=minikube,videoSplit
    fi
fi
